  ...
  <div>
    <div class="signup p-5" v-if="!!user">
      <form @submit="signup" v-if="user" novalidate v-bind:class="formClass">
        <h2>Welcome {{ user.google.wt.Ad }}</h2>
        ...
        <div class="form-group">
          <label class="form-check-label">
            <input type="checkbox" required="required" v-model="user.google.wt.terms" /> I accept the
  ...
  methods: {
    loginWithGoogle: async function () {
      try {
        ...
        const existingUser = await checkForUser(this.$store, user.wt.cu, userInfo.access_token);

        if (!existingUser) {
          this.$infoToast(`Welcome ${user.wt.fV}!`);
          userInfo.isNew = true;
        } else {
          this.$infoToast(`Welcome back ${user.wt.fV}!`);
          router.push('/');
        }
    ...
    signup: async function (e) {
      e.preventDefault();
      if (!this.user.google.wt.DoB) {
        this.errors.push('DoB');
        this.formClass = 'was-validated';
      }
      if (!this.user.google.wt.terms) {
      ...
      this.$store
        .dispatch({
          type: 'updateUser',
          user: {
            name: this.user.google.wt.Ad,
            email: this.user.google.wt.cu,
            dateofbirth: this.user.google.wt.DoB,
          },
